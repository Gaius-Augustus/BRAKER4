...
# Last modified on February October 12th 2025
#
# MODIFIED for BRAKER3 Rescue Workflow:
#   - Added -d/--output_dir argument to specify output directory for bad_genes.lst
#   - This prevents race conditions when multiple parallel jobs run simultaneously
#   - Original script from BRAKER3 container: /opt/Augustus/scripts/getAnnoFastaFromJoingenes.py
#
...
parser.add_argument('-d', '--output_dir', required=False, type=str, default='.',
                    help='Output directory for bad_genes.lst file (default: current directory)')
...
if len(bad_tx) > 0:
    import os
    bad_genes_path = os.path.join(args.output_dir, "bad_genes.lst")
    try:
        with open(bad_genes_path, "w") as bad_handle:
            bad_handle.write(
        ...
        print("Error: Failed to open file " + bad_genes_path + "!")
        exit(1)
    print("WARNING: The GTF file contained " + str(len(bad_tx)) + " gene(s) " +
          "with internal Stop codons (see file " + bad_genes_path + ").")
    if args.filter:
